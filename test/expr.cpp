...
Expr *ExprQuaternion::Magnitude() const {
    return ((w->Square())
                ->Plus(
                    (vx->Square())->Plus((vy->Square())->Plus((vz->Square())))))
        ->Sqrt();
}
...
double Expr::Eval() const {
    ...
    case Op::CONSTANT:
        return v;
    case Op::VARIABLE:
        ssassert(false, "Not supported yet");
...
Expr *Expr::PartialWrt(hParam p) const {
    ...
    switch (op) {
    case Op::PARAM_PTR:
        return From(p == parp->h ? 1 : 0);
    case Op::PARAM:
        return From(p == parh ? 1 : 0);
...
bool Expr::DependsOn(hParam p) const {
    ...
    int c = Children();
    if (c == 1)
        return a->DependsOn(p);
    if (c == 2)
        return a->DependsOn(p) || b->DependsOn(p);
    return false;
}
...
void Expr::Substitute(hParam oldh, hParam newh) {
    ...
    int c = Children();
    if (c >= 1)
        a->Substitute(oldh, newh);
    if (c >= 2)
        b->Substitute(oldh, newh);
}
...
std::string Expr::Print() const {
    ...
    switch (op) {
    case Op::PARAM:
        return ssprintf("param(%08x)", parh.v);
    case Op::PARAM_PTR:
        return ssprintf("param(p%08x)", parp->h.v);

    case Op::CONSTANT:
        return ssprintf("%.3f", v);
    case Op::VARIABLE:
        return "(var)";
...
ExprParser::Token ExprParser::Lex(std::string *error) {
    ...
    if (isupper(c)) {
        ...
        if (s == "sqrt") {
            ...
            *error =
                "'" + s + "' is not a valid variable, function or constant";
        }
    } else if (isdigit(c) || c == '.') {
...
int ExprParser::Precedence(Token t) {
    ssassert(t.type == TokenType::BINARY_OP || t.type == TokenType::UNARY_OP ||
                 t.type == TokenType::OPERAND,
             "Unexpected token type");

    if (t.type == TokenType::UNARY_OP) {
        return 30;
    } else if (t.expr->op == Expr::Op::TIMES || t.expr->op == Expr::Op::DIV) {
        return 20;
    } else if (t.expr->op == Expr::Op::PLUS || t.expr->op == Expr::Op::MINUS) {
        return 10;
...
bool ExprParser::Reduce(std::string *error) {
    ...
    case TokenType::UNARY_OP: {
        ...
        case Expr::Op::SQUARE:
            ...
            break;
        case Expr::Op::SIN:
            e = e->Times(Expr::From(PI / 180))->Sin();
            break;
        case Expr::Op::COS:
            e = e->Times(Expr::From(PI / 180))->Cos();
            break;
        case Expr::Op::ASIN:
            e = e->ASin()->Times(Expr::From(180 / PI));
            break;
        case Expr::Op::ACOS:
            e = e->ACos()->Times(Expr::From(180 / PI));
            break;
        default:
            ssassert(false, "Unexpected unary operator");
        }
        r.expr = e;
        break;
    }
...
bool ExprParser::Parse(std::string *error, size_t reduceUntil) {
    while (true) {
        ...
        case TokenType::PAREN_LEFT: {
            // sub-expression
            if (!Parse(error, /*reduceUntil=*/stack.size()))
                return false;
